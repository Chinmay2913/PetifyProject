...
<ul>
    ...
    bird: [
        ...
        {
            "name": "Lovebird",
            "image": "/breeds/birds/Lovebird.avif",
            "info": `
            <h3>About</h3>
            <ul>
        ...
        {
            "name": "Parrot",
            "image": "/breeds/birds/Parrot.avif",
            "info": `
            <h3>About</h3>
            <ul>
        ...
        {
            "name": "Parrotlet",
            "image": "/breeds/birds/Parrotlet.avif",
            "info": `
            <h3>About</h3>
            <ul>
...
function displayBreeds(animal) {
    const breeds = breedsData[animal];
    const breedsGrid = document.getElementById('breeds');
    ...
    if (breeds && breeds.length) {
        breeds.forEach(breed => {
            ...
            breedImage.src = breed.image;
            breedImage.alt = breed.name;
            breedImage.dataset.info = breed.info;
            breedImage.dataset.name = breed.name;
            breedImage.classList.add('breed-image');
            breedImage.addEventListener('click', () => displayBreedInfo(breed.name, breed.info));
            ...
            breedContainer.appendChild(breedImage);
            breedContainer.appendChild(breedName);

            // Append container to the breeds grid
            breedsGrid.appendChild(breedContainer);
        });

        document.getElementById('breeds-title').style.display = 'block';
    } else {
        console.error('No breeds found for', animal);
    }


    document.getElementById('breed-info-section').style.display = 'none';
}


// Function to display breed information when a breed image is clicked
function displayBreedInfo(breedName, breedInfo) {
    console.log(`Displaying info for breed: ${breedName}`); // Debugging line
    console.log(`Breed info: ${breedInfo}`); // Debugging line

    const breedInfoDiv = document.getElementById('breed-info');
    if (breedInfoDiv) { // Check if the element exists
        breedInfoDiv.innerHTML = `<h3>${breedName}</h3><p>${breedInfo}</p>`;
        document.getElementById('breed-info-section').style.display = 'block';
    } else {
        console.error('Element with ID "breed-info" not found');
    }
}

// Event listeners for animal icons
document.querySelectorAll('.animal-icon').forEach(icon => {
    icon.addEventListener('click', () => {
        const animal = icon.dataset.animal;
        displayBreeds(animal);
    });
});
